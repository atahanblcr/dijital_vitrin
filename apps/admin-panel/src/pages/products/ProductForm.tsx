...
const productSchema = z.object({
  ...
  short_desc: z.string().max(150, 'Kısa açıklama en fazla 150 karakter olabilir').optional(),
...
const ProductForm = ({ onBack, onSuccess }: { onBack: () => void, onSuccess: () => void }) => {
  const [categories, setCategories] = useState<{id: string, name: string}[]>([]);
  const [images, setImages] = useState<File[]>([]);
  const [submitErrors, setSubmitErrors] = useState<any[]>([]);

  const { register, handleSubmit, control, watch, formState: { errors, isSubmitting } } = useForm<ProductFormValues>({
    resolver: zodResolver(productSchema),
    defaultValues: { in_stock: true, is_active: true, is_campaign: false, attributes: [] }
  });
  ...
  const onSubmit = async (data: ProductFormValues) => {
    setSubmitErrors([]);
    try {
      // 1. Ürünü ve özelliklerini API'ye gönder (Dinamik validasyon API tarafında yapılacak)
      const res = await api.post('/business/products', data);
      const productId = res.data.data.id;

      // 2. Eğer görsel varsa FormData ile Image upload servisini çağır
      if (images.length > 0) {
      ...
      if (error.response?.data?.details) {
        ...
        toast.error(error.response?.data?.error || 'Ürün kaydedilemedi');
      }
    }
  };
    ...
    <div className="max-w-4xl mx-auto pb-12">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-6">
          ...
          <ImageUploader images={images} onChange={setImages} maxFiles={7} />
        </div>
        ...
        >
          ...
          {isSubmitting ? 'Kaydediliyor...' : 'Ürünü Kaydet ve Yayınla'}
        </button>
      </form>
    </div>
  );
